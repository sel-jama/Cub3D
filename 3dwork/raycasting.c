...
static void	set(t_ray *ray, t_player *player)
{
	if (ray->dir_x < 0)
	{
		ray->step_x = -1;
...
/*
- We implement the  algorithm -> the loop will increment 1 square 
-   until we hit a wall
- If the sidedistx < sidedisty, x is the closest point from the ray
*/

void	perform(t_data *data, t_ray *ray)
{
	int	hit;
